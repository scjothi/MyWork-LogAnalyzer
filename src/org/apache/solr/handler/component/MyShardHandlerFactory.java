...
public class MyShardHandlerFactory extends ShardHandlerFactory implements
		org.apache.solr.util.plugin.PluginInfoInitialized
{
	protected static Logger log = LoggerFactory.getLogger(MyShardHandlerFactory.class);
	...
	{
		log.info("XXXXXXX123XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX");
	...
	public ShardHandler getShardHandler(final HttpClient httpClient)
	{
		...
		HttpRequestInterceptor authInterceptor = new HttpRequestInterceptor()
		{
			...
			public void process(final HttpRequest request, final HttpContext context) throws HttpException, IOException
			{
				...
				log.info("XXXXXXXX CREDS XXXXX...1");
				...
				if (authState.getAuthScheme() == null)
				{
					...
					log.info("XXXXXXXX CREDS XXXXX...2");
					if (authScheme != null)
					{
						...
						log.info("XXXXXXXX CREDS" + creds.getUserPrincipal().getName());
		...
		HttpClientUtil.setBasicAuth(dhttpClient, "user123", "pass123");
		dhttpClient.addRequestInterceptor(authInterceptor, 0);

		return new MyShardHandler(this, dhttpClient);
	}
	...
	public void init(PluginInfo info)
	{
		...
		BlockingQueue<Runnable> blockingQueue = (this.queueSize == -1) ? new SynchronousQueue<Runnable>(
				this.accessPolicy) : new ArrayBlockingQueue<Runnable>(this.queueSize, this.accessPolicy);
		...
		this.defaultClient = (DefaultHttpClient) HttpClientUtil.createClient(clientParams);
