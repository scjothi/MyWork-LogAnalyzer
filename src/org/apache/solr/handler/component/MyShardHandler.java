...
public class MyShardHandler extends ShardHandler
{
	...
	private MyShardHandlerFactory httpShardHandlerFactory;
	...
	public MyShardHandler(MyShardHandlerFactory httpShardHandlerFactory, HttpClient httpClient)
	{
	...
	public void checkDistributed(ResponseBuilder rb)
	{
		...
		rb.isDistrib = params.getBool("distrib", req.getCore().getCoreDescriptor().getCoreContainer()
				.isZooKeeperAware());
		...
		if (rb.isDistrib)
		{
			...
			else if (zkController != null)
			{
				...
				if (shardKeys == null)
				{
					shardKeys = params.get(ShardParams.SHARD_KEYS);//eprecated
				}
					...
					if (rb.shards[i] == null)
					{
						...
						for (Replica replica : sliceShards.values())
						{
							if (!clusterState.liveNodesContain(replica.getNodeName())
									|| !replica.getStr(ZkStateReader.STATE_PROP).equals(ZkStateReader.ACTIVE))
							{
								continue;
							}
							if (first)
	...
	/**
	 * returns a ShardResponse of the last response correlated with a ShardRequest. This won't return early if it runs
	 * into an error.
	 **/
	@Override
	...
	/**
	 * returns a ShardResponse of the last response correlated with a ShardRequest, or immediately returns a
	 * ShardResponse if there was an error detected
	 */
	@Override
