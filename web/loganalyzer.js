...
/*
 * $(function() { $( "#datepicker1" ).datepicker(); $( "#datepicker2"
 * ).datepicker(); });
 */
...
{
    var fdate= new Date(); fdate.setSeconds(0); fdate.setMilliseconds(0);
    var tdate= new Date(); tdate.setSeconds(0); tdate.setMilliseconds(0);
    var today = new Date(); today.setSeconds(0); today.setMilliseconds(0);
...
// 2013-08-08T00:00
    ...
    if(from != ''){
        ...
        // disable distributed search if from date is today
		if(today<=fdate)
		{
  ...
  // alert('called');
    ...
    var applySt="";
     ...
     for(i=recsize;i>0;)
     {
      ...
      // alert(html);
/*
 * var pages = html.split("|"); //alert(pages);
 * 
 * var pageCount = pages.length; //alert(ll); var currPg =
 * Math.floor(_start/recct); var startPg = currPg-5; startPg = (startPg<0 ||
 * pageCount<10) ? 0:startPg; //var en = cn+5; en = (en>ll) ? ll:en;
 * 
 * alert(startPg+':'+currPg); html=''; bbb: for(x=startPg;x<startPg+10;x++) {
 * html = html + '|' + pages[x]; if(x>=pageCount-1) { break outer; } }
 */
...
if(str.indexOf("\t")!=-1){
 ...
 for (var i = 0; i < array.length; i++) {
     ...
     // token that has a 'caused by'
       ...
       // if this 'caused by' occured within 5th line from the begin or from
		// the previous 'caused by'
       if(i<causedByIndex+4)
       {
        // result = result+'<br><span class="tab"/>'+array[i].replace("\tCaused
		// by:", "<br>Caused by:");
...
/*
 * 
 * if(i==causedByIndex+4){ result = result+'<div id="a'+vnum+'" class="abc" ><span
 * class="tab"/>'+array[i]; } else if(i==array.length-1){ result = result+'<br><span
 * class="tab"/>'+array[i]+'</div>'; } else if(i==0){ result = result+array[i]; }
 * else if(i>0 && array[i].indexOf("Caused by:") == -1){
 * 
 * result = result+'<br><span class="tab"/>'+array[i]; } else if(i>0 &&
 * array[i].indexOf("Caused by:") != -1){ result = result+'</div><br>'+array[i];
 * causedByIndex = i; }
 * 
 */
 }
...
return str;
        ...
        // alert(objArray);
            ...
            for (var i = 0; i < array.length; i++) {
                ...
                for (var index in array[i]) {
                    ...
                    line += array[i][index];
                    line = line.replace(/</g, '&lt;'); // REPLACING ALL '<' TO
														// '&LT;'
                    line = replacePipe(line, i);
                ...
                // alert(hrf);
                eventxt += '<tr><td valign="top">'+i+'</td><td>'+   line + '<br><a href="'+hrf+'">Show All</a>'   +   '</td></tr>';
            }


            return "<table id='page1' width='100%' border='0'>"+eventxt+"</table>";
        }
     ...
     function ExecuteSearch(url)
     {
	  ...
	  else if(searchOnlyArchive)
	  {
        url = _archiveUri+url;
	  }
	  else
	  {
		url = _uri+_shardParam+url;
	  }

	  // reset search switches
	  searchOnlyCurrent=false;
	  searchOnlyArchive=false;

    // $.getJSON("http://localhost:7070/solr-example/collection1/select?q=LEVEL%3AERROR&rows=1500&fl=id%2CSERVER%2CINSTANCE%2CWEBSERVICE%2CTIMESTAMP%2CLEVEL%2CTHREAD%2CCLASS%2CMESSAGE%2C_version_&wt=json&indent=true",
	// function(rec) {
   ...
   $('#spinner').show();
    ...
    if(key == "response")
    {
      ...
      // INCREASING THE RECORDS PER PAGE LENGTH IF MORE THAN 1 MILLION MATCHES
		// FOUND
      /*
		 * if(numFound > 1e6){ _rows = 10000; }
		 */

     // $('div.results').html('<tr><td width="100%" bgcolor="white"
		// colspan="5" >Number of Records: '+numFound +getPages(numFound)+
		// '</td></tr>');
 ...
 $('#spinner').hide();
        ...
        "TRANSACTION": "e4453239-73b4-4d96-bbfb-af7e9fcf519a",
...
function exportTableToCSV($table, filename) {
	...
	var header = 'RECORD,LINE,INSTANCE,ENV,WEBSERVICE,TRANSACTION,DATE,LEVEL,THREAD,CLASS,MESSAGE\r\n';
	...
	if($table.length == 0)
        ...
        var $rows = $table.find('tr:has(td)'),
            ...
            // Data URI
            csvData = 'data:application/csv;charset=utf-8,' +  encodeURIComponent(header + csv);
